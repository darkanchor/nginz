...
**Verdict: S0/S1 CORE FIXED (2026-07-12).** RESP bulk/array/integer parsing uses checked slice arithmetic, overflow-safe numeric parsing, exact CRLF validation, a 32 KiB value limit, an exact 16-item MGET contract, and controlled 502 responses. JSON rendering computes escaped size and writes to bounded request-pool storage up to 256 KiB. The upstream frame buffer now matches that enforced aggregate bound with framing headroom, so fragmented legal frames cannot exhaust the former 32 KiB header buffer; an exact-limit 32 KiB value regression is green. Larger streaming is a feature/performance extension, not an S1 correctness dependency.
