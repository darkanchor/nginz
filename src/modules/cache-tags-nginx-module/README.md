...
# Response
{"tag":"user-123","purged":5,"physical_cache_invalidated":false}
```

Listing `GET /cache/purge` returns a `capture_rejections` object with cumulative
`invalid`, `tag_capacity`, and `uri_capacity` counts. These counters make
bounded-store pressure visible; they do not make a metadata purge proof of
physical nginx cache-file eviction.
...
- Keep tag names and URI paths reasonably short. The store rejects tag names over 64 bytes and URIs over 256 bytes before they consume shared slots, and increments `capture_rejections.invalid`.
- The store never evicts live mappings to make room. Tag names beyond `MAX_TAGS` (256) increment `capture_rejections.tag_capacity`; URIs beyond `MAX_URIS_PER_TAG` (64) per tag increment `capture_rejections.uri_capacity`. Purge stale metadata regularly or provision a larger implementation for high-cardinality workloads.
...
- **Memory Only**: Tags are stored in nginx shared memory, survive a graceful reload, and are lost on a full nginx restart
- **No Wildcards Yet**: Pattern matching limited to exact tags
- **Fixed Capacity**: The current shared-memory store uses fixed tag and URI limits (`MAX_TAGS=256`, `MAX_URIS_PER_TAG=64`, `MAX_URI_LEN=256`); rejected captures are counted by the listing endpoint
...
**Verdict: S1 CAPACITY/RELOAD FIXED.** Oversized tags/URIs and full tag/URI tables are rejected without overwriting live mappings, with cumulative rejection counters exposed by the listing API. The listing response is bounded for a full table and JSON-escapes stored tag text. The focused 19-case shared-worker suite proves saturated metadata and counters survive graceful reload. Purge responses explicitly state `physical_cache_invalidated:false`: this module removes tag metadata only and does not claim to evict nginx cache files.
