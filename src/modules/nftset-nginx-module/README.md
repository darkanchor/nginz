...
**Verdict: S1 AVAILABILITY BOUNDED.** Both shared zones are refreshed per cycle and shared mutations are locked; the L1/L2 generation design is coherent. Rate limits use a shared rolling 1,000 ms fixed window anchored at the first request, avoiding epoch-boundary budget resets during a burst. Netlink remains synchronous in the HTTP access path, but each receive is capped at 10 ms (a confirmed miss can require two exchanges), replacing the former 100 ms exposure. The 33-case mock suite and 35-case live nftables suite are green. Fully asynchronous Netlink and additional metrics remain performance/observability work.
