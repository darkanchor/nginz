...
function buildNginzConfig(port) {
  ...
  const config = [
    ...
    `        listen ${port};`,
    "",
    "        location /bench/dynamic-upstreams {",
...
async function main() {
  if (process.argv.includes("--help")) {
    printBenchmarkHelp(import.meta.path, "nginz dynamic-upstreams module");
    process.exit(0);
  }
  ...
  for (const scenario of scenarios) {
    for (const c of concurrencies) {
      ...
      results.push({ service: "nginz-dynamic-upstreams", scenario: scenario.name, concurrency: c, summary });
