...
async function main() {
  if (process.argv.includes("--help")) {
    printBenchmarkHelp(import.meta.path, "nginz worker-events module");
    process.exit(0);
  }
  ...
  for (const scenario of scenarios) {
    for (const c of concurrencies) {
      ...
      results.push({ service: "nginz-worker-events", scenario: scenario.name, concurrency: c, summary });
  ...
  writeJsonArtifact(activeArtifacts.commandPath, captureCommandArtifact("perf/worker-events/benchmark/run.js", options));
