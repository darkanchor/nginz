...
import { createRedisMock } from "../../../tests/harness.js";

const MODULE = "redis";
const REDIS_PORT = 16379;
const PERF_DIR = join(process.cwd(), "perf", "redis");
...
function buildNginzConfig() {
  ...
  const config = [
    ...
    "",
    "        location /bench/small {",
    `            redis_pass 127.0.0.1:${REDIS_PORT};`,
    "        }",
    "",
    "        location /bench/medium {",
    `            redis_pass 127.0.0.1:${REDIS_PORT};`,
    "        }",
    "",
    "        location /bench/large {",
    `            redis_pass 127.0.0.1:${REDIS_PORT};`,
    "        }",
    "",
    "        location /bench/static {",
    `            redis_pass 127.0.0.1:${REDIS_PORT};`,
    "            redis_key static-value;",
    "        }",
    "    }",
    "}",
  ].join("\n");
...
function getNginzPid() {
  try {
    const pidPath = join(activeRuntime.dir, "logs", "nginx.pid");
...
async function measureSamples({ url, requests, concurrency, warmup }) {
  if (warmup > 0) {
...
async function main() {
  if (process.argv.includes("--help")) {
    printBenchmarkHelp(import.meta.path, "nginz redis module");
    process.exit(0);
  }
  ...
  const smallVal = "benchmark-value-42";
  redisMock.setValue("bench/small", smallVal);
  redisMock.setValue("bench/medium", "x".repeat(1024));
  ...
  for (const scenario of scenarios) {
    for (const c of concurrencies) {
      ...
      const summary = await measureSamples({ url, requests: options.requests, concurrency: c, warmup: options.warmup });

      results.push({ service: "nginz-redis", scenario: scenario.name, concurrency: c, summary });
  ...
  writeJsonArtifact(activeArtifacts.commandPath, captureCommandArtifact("perf/redis/benchmark/run.js", options));
