...
describe("worker-events Phase 1 - publish and inspect", () => {
  ...
  test("returns 405 for unsupported methods", async () => {
    for (const method of ["PUT", "DELETE", "PATCH"]) {
      const res = await fetchClose(`${TEST_URL}/worker-events`, { method });
  ...
  test("HEAD returns headers with no body", async () => {
    const res = await fetchClose(`${TEST_URL}/worker-events`, { method: "HEAD" });
    expect(res.status).toBe(200);
    expect(res.headers.get("content-type")).toContain("application/json");
  ...
  test("GET inspect supports filtering by event type", async () => {
    ...
    const res = await fetchClose(`${TEST_URL}/worker-events?type=beta_event`);
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  test("inspect with limit= caps response size", async () => {
    const res = await fetchClose(`${TEST_URL}/worker-events?limit=2`);
    const body = await res.json();
  ...
  test("inspect with channel= filters events", async () => {
    ...
    const res = await fetchClose(`${TEST_URL}/worker-events?channel=cache.invalidate`);
    const body = await res.json();
  ...
  test("inspect with non-matching channel returns empty", async () => {
    const res = await fetchClose(`${TEST_URL}/worker-events?channel=nonexistent`);
    const body = await res.json();
  ...
  test("event types are JSON-escaped in inspect output", async () => {
    ...
    const inspect = await fetchClose(`${TEST_URL}/worker-events`);
...
describe("worker-events Phase 1 - error config", () => {
  ...
  test("small ring publish and inspect works as an independent zone", async () => {
    const res = await fetchClose(`${TEST_URL}/small-ring`, {
      method: "POST",
      headers: { "content-type": "application/json" },
    ...
    const inspect = await fetchClose(`${TEST_URL}/small-ring`);
    expect(inspect.status).toBe(200);
...
describe("worker-events Phase 1 - multi-zone isolation", () => {
  ...
  test("separate zones keep separate generation counters and events", async () => {
    const aPub = await fetchClose(`${TEST_URL}/zone-a`, {
      method: "POST",
      headers: { "content-type": "application/json" },
    ...
    const bPub = await fetchClose(`${TEST_URL}/zone-b`, {
      method: "POST",
      headers: { "content-type": "application/json" },
    ...
    const aInspect = await fetchClose(`${TEST_URL}/zone-a`);
    ...
    const bInspect = await fetchClose(`${TEST_URL}/zone-b`);
