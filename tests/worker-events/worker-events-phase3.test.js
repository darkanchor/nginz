...
describe("worker-events Phase 3 - publish authorization", () => {
  ...
  test("publish with wrong key returns 401", async () => {
    const res = await fetch(`${TEST_URL}/worker-events?key=wrongkey`, {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  test("publish with correct key succeeds", async () => {
    const res = await fetch(`${TEST_URL}/worker-events?key=secret123`, {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  test("key comparison is exact (substring fails)", async () => {
    const res = await fetch(`${TEST_URL}/worker-events?key=secret1234`, {
      method: "POST",
      headers: { "content-type": "application/json" },
...
describe("worker-events Phase 3 - introspection fields", () => {
  ...
  test("inspect returns all observable fields", async () => {
    // Publish first
    await fetch(`${TEST_URL}/worker-events?key=secret123`, {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  test("last_publish_msec updates after each publish", async () => {
    const before = await fetch(`${TEST_URL}/worker-events`);
    ...
    const after = await fetch(`${TEST_URL}/worker-events`);
